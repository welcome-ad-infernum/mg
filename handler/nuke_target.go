...
func NukeTarget(
	...
	return func(ctx context.Context, w pipelines.EventWriter[dto.TargetResponse], target dto.Target) {
		select {
		case <-ctx.Done():
			return
		default:
		...
		if err != nil {
			w.Write(pipelines.NewErrEvent[dto.TargetResponse](
		...
		for i := amountRequests; i > 0; i-- {
			...
			if errors.Is(err, context.DeadlineExceeded) || os.IsTimeout(err) {
				...
				w.Write(pipelines.NewErrEvent[dto.TargetResponse](
					pipelines.NewError(errors.Wrap(err, "target is down"), true),
				))
			...
			if errors.Is(err, io.EOF) ||
				...
				w.Write(pipelines.NewErrEvent[dto.TargetResponse](
					pipelines.NewError(errors.Wrap(err, "target filters traffic"), true),
				))
			...
			if err != nil {
				...
				w.Write(pipelines.NewErrEvent[dto.TargetResponse](
					pipelines.NewError(errors.Wrap(err, "request failed"), true),
				))
